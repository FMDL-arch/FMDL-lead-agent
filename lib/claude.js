...
async function getAgentReply(messages, extraContext = '') {
  ...
  const locationMatch = text.match(/\[\[LEAD_LOCATION:\s*(.+?)\]\]/);
  text = text.replace(/\[\[LEAD_LOCATION:.+?\]\]/, '').trim();

  const jobFormSentMatch = text.match(/\[\[JOB_FORM_SENT\]\]/);
  text = text.replace(/\[\[JOB_FORM_SENT\]\]/, '').trim();

  const jobFormConfirmedMatch = text.match(/\[\[JOB_FORM_CONFIRMED\]\]/);
  text = text.replace(/\[\[JOB_FORM_CONFIRMED\]\]/, '').trim();
  ...
  return {
    ...
    leadLocation: locationMatch ? locationMatch[1].trim() : null,
    jobFormSent: !!jobFormSentMatch,
    jobFormConfirmed: !!jobFormConfirmedMatch,
  };
}
